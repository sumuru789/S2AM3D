...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, prompt_feat, return_context=False):
        ...
        context = (attn @ v).transpose(1, 2).reshape(B, M, C)             
...
class CrossAttentionPointsQ(nn.Module):
    ...
    def forward(self, x, prompt_feat):
        ...
        k = self.k_proj(prompt_feat).reshape(B, M, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)             
        v = self.v_proj(prompt_feat).reshape(B, M, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)             
...
class CrossAttentionDecoder(nn.Module):
    ...
    def forward(self, x, prompt_feat):
        ...
        x_norm = self.norm1(x)
        context = self.attn(x_norm, prompt_feat, return_context=True)           
        prompt_feat = residual_prompt + self.drop_path(context)

                                                  
        residual_x = x
        out_points = self.attn_points_q(x_norm, prompt_feat)           
