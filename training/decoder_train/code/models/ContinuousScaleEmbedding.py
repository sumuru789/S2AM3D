...
class ContinuousScaleEmbedding(nn.Module):
    ...
    def forward(self, scale_ratio):
        ...
        freqs = self.scale_freqs.unsqueeze(0) * scale_normalized.unsqueeze(1)                   
        phases = self.scale_phases.unsqueeze(0)                   
        
        sin_encodings = torch.sin(freqs + phases)                   
        cos_encodings = torch.cos(freqs + phases)                   
        
                   
        scale_encoding = torch.cat([sin_encodings, cos_encodings], dim=-1)                     
        
                 
        scale_embedding = self.scale_proj(scale_encoding)                    
