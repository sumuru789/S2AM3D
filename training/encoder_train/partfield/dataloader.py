...
class Demo_Remesh_Dataset(torch.utils.data.Dataset):
    ...
    def get_model(self, ply_file):
        ...
        try:
            ...
            new_mesh.vertices = new_mesh.vertices * (2.0 / size) - 1.0
...
class PartNetDataset(torch.utils.data.Dataset):
    def __init__(self, metadata_path, num_points=100000, is_train=True, source_name=None):
        ...
        if source_name is not None:
            ...
            if 'partnet' in lower_path:
                ...
                self.source = 'unknown'
        print(f"{'train' if is_train else 'test'}: {len(self.metadata)}")
    ...
    def __getitem__(self, index):
        ...
        result = {
            ...
            'color': color if isinstance(color, torch.Tensor) else torch.tensor(color, dtype=torch.float32),
            'uid': model_id,
